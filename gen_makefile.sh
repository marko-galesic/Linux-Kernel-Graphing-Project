...
echo 'image-unop.ps: outline.ps ../key.ps ../authors.ps ../place.sh ../title.ps ring1.ps ring2.ps ring3.ps ring4.ps'
echo '	cat ring[1234].ps > $@'
echo '	../place.sh bottom-right ../authors.ps $@ >> $@'
echo '	../place.sh bottom-left ../key.ps $@ >> $@'
echo '	../place.sh above ../title.ps $@ >> $@'
# Bounding box is at bottom now.  Next two won't change it.
echo '	tail -n 1 $@ > bounding-box'
echo '	cat outline.ps >> $@'
echo '	cat ../tux.ps >> $@'
...
echo RING4_DEPS:=`for d in $RING4; do echo $d-ring4.ps; done`
...
echo "	@echo /angle 0 def > \$@"
echo "	@../rotary_arrange.sh $DIR_SPACING" `for f in $RING2; do echo $f-ring2.ps $f-ring2.angle; done | sort` '>> $@'
...
echo "	@echo /angle 0 def > \$@"
echo "	@../rotary_arrange.sh $DIR_SPACING" `for f in $RING3; do echo $f-ring3.ps $f-ring3.angle; done | sort` '>> $@'
echo "	@echo Done Ring 3"

# Outer ring starts at end of fourth ring.
# And it's just a big ring of drivers.
echo 'ring4.ps: ring3.ps $(RING4_DEPS)'
echo "	@echo Making Ring 4"
echo "	@echo /angle 0 def > \$@"
#echo "	@../draw_arrangement $FILE_SCRUNCH 0 360 \`cat ring3.radius\` \$(RING4_DEPS) >> \$@"
echo "	@../rotary_arrange.sh $DIR_SPACING" `for f in $RING4; do echo $f-ring4.ps $f-ring4.angle; done | sort` '>> $@'
...
echo "	@echo Rendering \$@"
echo "	@../draw_arrangement $FILE_SCRUNCH 0 \`cat \$<\` \`cat ring1.radius\` $SMALL_ROW_SCRUNCH $SMALL_ROW_MAX \`find \$* -name '*-all.ps'\` > \$@"

echo "%-ring3.ps: %-ring3.angle ring2.radius"
echo "	@echo Rendering \$@"
echo "	@../draw_arrangement $FILE_SCRUNCH 0 \`cat \$<\` \`cat ring2.radius\` $SMALL_ROW_SCRUNCH $SMALL_ROW_MAX \`find \$* -name '*-all.ps'\` > \$@"

echo "%-ring4.ps: %-ring4.angle ring3.radius"
echo "	@echo Rendering \$@"
echo "	@../draw_arrangement $FILE_SCRUNCH 0 \`cat \$<\` \`cat ring3.radius\` $SMALL_ROW_SCRUNCH $SMALL_ROW_MAX \`find \$* -name '*-all.ps'\` > \$@"

# How to extract radii
echo "%.radius: %.ps"
echo '	@echo scale=2\; `tail -n 1 $< | sed "s/^.* //"` + '$RING_SPACING' | bc > $@'
echo
...
echo "%-ring4.angle: %-ring4.weight ring4.weight"
echo '	@echo "scale=2; ( 360 - ' `echo $RING4 | wc -w` ' * ' $DIR_SPACING ') * `cat $<` / `cat ring4.weight`" | bc > $@'
...
echo '	@cat $^ | ../tally > $@'
echo "ring4.weight:" `for d in $RING4; do echo $d-ring4.weight; done`
echo '	@cat $^ | ../tally > $@'
...
echo '	@../total_area.pl `find $* -name \*-all.ps` > $@'
echo "%-ring4.weight:" `find $RING4 -name '*.c.*' | sed 's/\.c.*/-all.ps/' | sort | uniq`
echo '	@../total_area.pl `find $* -name \*-all.ps` > $@'
echo
...
echo outline.ps: ../make-outline.sh ring1.ps ring2.ps ring3.ps ring4.ps `for f in $RING2; do echo $f-ring2.angle; done` `for f in $RING3; do echo $f-ring3.angle; done` `for f in $RING4; do echo $f-ring4.angle; done`
